...
# **peers_peerid_post**
> Peer peers_peerid_post(peerid, add=add)
...
api_instance = openapi_client.DefaultApi()
peerid = 4f196836-0d9d-475a-aae2-642bb0eac685 # str | Peer ID
add = {$ref=./req_peer_example.yml#/add} # Add |  (optional)

try:
    api_response = api_instance.peers_peerid_post(peerid, add=add)
    pprint(api_response)
except ApiException as e:
...
try:
    api_response = api_instance.peers_post(add=add)
    pprint(api_response)
except ApiException as e:
...
try:
    api_response = api_instance.volumes_post(add=add)
    pprint(api_response)
except ApiException as e:
